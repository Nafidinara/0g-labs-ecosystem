...
from web3connect import connect  # Impor fungsi connect dari web3connect.py
...
def compile_contract(language: str = 'en'):
    ...
    # Compile with specific settings as command line arguments
...
async def deploy_contract(w3: Web3, private_key: str, wallet_index: int, name: str, symbol: str, decimals: int, total_supply: int, language: str = 'en'):
    ...
    try:
        ...
        print(f"{Fore.YELLOW}  - Gas limit được đặt cố định: {gas_limit}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}  - Gas price được đặt cố định: {w3.from_wei(gas_price, 'gwei')} gwei{Style.RESET_ALL}")
        ...
        # print signed_tx
        print(f"{Fore.YELLOW}  - Tx hash: {signed_tx.hash.hex()}{Style.RESET_ALL}")
        tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
        tx_link = f"{EXPLORER_URL}/tx/0x{tx_hash.hex()}"

        # Đợi receipt
        receipt = await asyncio.get_event_loop().run_in_executor(None, lambda: w3.eth.wait_for_transaction_receipt(tx_hash, timeout=500))
